...
`
  ...
  a.href = url
  a.download = `RakshaSetu_Report_${caseItem.id}.txt`
...
export default function CaseStatusPage() {
  ...
  useEffect(() => {
    ...
    setUser(currentUser)
    setCases(store.getUserCases(currentUser.id))

    const unsubscribe = store.subscribe(() => {
      const updatedUser = store.getUser()
      if (updatedUser && updatedUser.id === currentUser.id) {
        setCases(store.getUserCases(updatedUser.id))
      }
    })
                      ...
                      <Dialog key={caseItem.id}>
                        <div className="flex items-center gap-4 rounded-lg border border-border bg-card p-4">
                          ...
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => downloadCaseReport(caseItem)}
