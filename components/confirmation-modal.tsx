...
const generatePDFReport = (caseData: Case) => {
  const reportContent = `
...
Submitted: ${new Date(caseData.submittedAt).toLocaleString("en-IN")}

REPORTER DETAILS
────────────────
Name: ${caseData.userName}
Role: ${caseData.userRole}
...
${caseData.affectedSystem ? `Affected System: ${caseData.affectedSystem}` : ""}
${caseData.location ? `Location: ${caseData.location}` : ""}

EVIDENCE FILES
────────────────
${caseData.evidence.map((file, i) => `${i + 1}. ${file}`).join("\n")}

MITIGATION STEPS
────────────────
${caseData.mitigationSteps ? caseData.mitigationSteps.map((step, i) => `${i + 1}. ${step}`).join("\n") : "No mitigation steps provided"}
...
`
  ...
  a.href = url
  a.download = `RakshaSetu_Report_${caseData.id}.txt`
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="flex gap-3">
          <Button onClick={() => generatePDFReport(caseData)} variant="outline" className="flex-1 bg-transparent">
